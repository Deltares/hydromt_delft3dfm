...
def test_reduce_gdf_precision():
    ...
    unsupported_type = gpd.GeoDataFrame(
        geometry=[MultiPoint([Point(0, 1), Point(10, 2)])],
        data={"crsid": [1], "order": [1], "z": [10]}, crs=28992)
    unsupported_type.index = unsupported_type.index.astype(str)
    ...
    with pytest.raises(NotImplementedError) as e:
        _ = reduce_gdf_precision(unsupported_type)
    assert "Only LineString and Point geometry types supported." in str(e.value)
